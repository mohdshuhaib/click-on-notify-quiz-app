...
export async function saveFullQuiz(quizData: any, questionsData: any[]) {
  ...
  try {
    ...
    for (let i = 0; i < questionsData.length; i++) {
      ...
      const questionTextToSave = (q.statements && q.statements.length > 0)
        ? JSON.stringify({ _type: 'complex', text: q.text, statements: q.statements })
        : q.text;
...
export async function updateFullQuiz(quizId: string, quizData: any, questionsData: any[]) {
  ...
  try {
    ...
    // Build bulk arrays to reduce DB hits
    const questionsToUpsert = questionsData.map((q, i) => {
      const questionTextToSave = (q.statements && q.statements.length > 0)
        ? JSON.stringify({ _type: 'complex', text: q.text, statements: q.statements })
        : q.text;
    ...
    for (const q of questionsData) {
      ...
      allOptionsToUpsert.push(...optionsToUpsert);
    }

    if (allOptionsToUpsert.length > 0) {
      const { error: optError } = await supabase
        .from('options')
        .upsert(allOptionsToUpsert, { onConflict: 'id' });
      if (optError) throw optError;
    }
    ...
    revalidatePath('/admin/quiz-settings');
    revalidatePath(`/admin/quiz-settings/${quizId}/edit`);
...
export async function getQuizExportData(quizId: string) {
  ...
  try {
    ...
    // Format complex questions for export
      ...
      try {
        if (parsedText.startsWith('{') && parsedText.includes('"_type"')) {
          ...
          if (parsed._type === 'complex') {
            parsedText = parsed.text + (parsed.statements ? ' (' + parsed.statements.join(' | ') + ')' : '');
          }
        }
      } catch (e) {}
      return { ...q, question_text: parsedText };
    }) || [];
    
    // Extract options from the nested query
    ...
    return { success: true, quiz, questions: formattedQuestions, options: options, submissions: submissions || [] };
  } catch (error: any) {
